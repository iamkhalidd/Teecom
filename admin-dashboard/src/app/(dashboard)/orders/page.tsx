...
import { Search, Filter, Eye, CheckCircle, Truck, Download, ShoppingBag as ShoppingBagIcon } from "lucide-react"
    ...
    <div className="relative p-8 space-y-8">
      ...
      <div className="flex gap-4">
        ...
        <Button
          ...
          onClick={() => window.open(process.env.NEXT_PUBLIC_API_URL + "/orders/export_csv/", "_blank")}
        >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {orders.map((order) => (
            <div
              key={order.id}
              className={`bg-card p-4 rounded-2xl flex items-center justify-between shadow-sm transition-all ${selectedIds.includes(order.id) ? "ring-2 ring-primary" : ""}`}
            >
